...
ringbahn_names_df <- base::data.frame(name = ringbahn_names, stringsAsFactors = FALSE) %>% 
...
ringbahn_df <- stations_df %>%
  ...
  dplyr::filter(fclass %like% "railway" & name %in% ringbahn_names) %>%
  dplyr::group_by(name) %>%
...
leaflet() %>% 
  ...
  addLabelOnlyMarkers(data = districts,
                    lng = ~long, lat = ~lat, label = ~lapply(leaflet, html),
