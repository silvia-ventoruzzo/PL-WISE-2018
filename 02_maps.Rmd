...
# Load the shapefile
```{r}
berlin <- shapefile(file.path(getwd(), "mygeodata", "Berlin-Ortsteile-polygon.shp", fsep="/"))
# Calculating the center of Berlin
berlin_center <- slot(berlin, "bbox") %>%
  as.data.frame() %>%
  transmute(center = (max + min)/2,
            coords = c("long", "lat")) %>%
  spread(key = coords, value = center)
```
...
```{r}
berlin_district_sf <- unionSpatialPolygons(berlin, berlin$BEZNAME) %>% # Join polygons according to district name
  st_as_sf() %>% # Transform into sf Object
  st_set_crs(proj4string(berlin)) %>% # add Coordinate Reference System (CRM)
  mutate(id = berlin$BEZNAME %>% unique() %>% sort())
```
...
districts <- broom::tidy(berlin, region = "BEZNAME") %>%
  ...
  mutate(group = replace(group, values = id)) %>%
  mutate(group = replace(group, values = gsub("-", "-<br/>", group))) %>%
  group_by(group, id) %>%
...
leaflet() %>%
  ...
  addLabelOnlyMarkers(data = districts,
                    lng = ~long, lat = ~lat, label = ~lapply(group, htmltools::HTML),
...
```{r}
berlin_all_sf <- aggregate(berlin,dissolve=TRUE) %>%
  st_as_sf() %>%
  st_set_crs(proj4string(berlin))
```
...
```{r}
stations <- shapefile(file.path(getwd(), "berlin-latest-free", "gis_osm_transport_free_1.shp", fsep="/"))
```
...
stations_df <- data.frame(stations) %>% 
  ...
  mutate(name = replace(name, values = gsub("Berlin ", "", name)),
        name = replace(name, values = gsub("Berlin-", "", name)),
        name = replace(name, values = gsub(" *\\(.*?\\) *", "", name))) %>%
  dplyr::select(fclass, name, long, lat)
  
# Load the names of all Ringbahn stations, the initial one repeated at the end so we can get a full circle
ringbahn_names <- c("Südkreuz", "Schöneberg", "Innsbrucker Platz", "Bundesplatz",
...
ringbahn_names_df <- data.frame(name = ringbahn_names, stringsAsFactors = FALSE) %>% 
  mutate(order = 1:n()) 
...
ringbahn_df <- stations_df %>%
  mutate(name = replace(name, values = gsub("S ", "", name)),
        name = replace(name, values = gsub("U ", "", name))) %>%
  filter(fclass %like% "railway" & name %in% ringbahn_names) %>%
  group_by(name) %>%
  dplyr::summarize(long = mean(long), lat = mean(lat)) %>%
  full_join(ringbahn_names_df, by = "name") %>%
  arrange(order) 
```
...
ringbahn_sf <- SpatialPolygons(list(Polygons(list(Polygon(ringbahn_coordinates)),
                                             ID = "A")),
                               proj4string = CRS(proj4string(berlin))) %>% # Transform into spatial polygon
  st_as_sf() %>%
  mutate(id = "A") # Transform into sf Object
```

# Create sf Object with areas A and B
Firstly we need to create an sf object with a hole for B
```{r}
vbb_B_sf <-  rbind(ringbahn_sf$geometry, berlin_all_sf) %>%
  st_intersection() %>% # Calculate intersections
  filter(n.overlaps == 1) %>% # Select only polygon where there is only one intersection
  select(geometry) %>%
  mutate(id = "B")
```
...
```{r}
vbb_AB_sf <- rbind(ringbahn_sf, vbb_B_sf)
```
...
I'm first trying this with price_avg
```{r}
# I don't like these colours
pal_districts <- colorNumeric(palette = "Dark2", domain = Listings_District$price_avg, n = 12, reverse = TRUE)
leaflet() %>% 
  addTiles() %>%
  addPolygons(data = berlin_district_sf,
            stroke = FALSE,
            fillColor = ~pal_districts(Listings_District$price_avg)) %>%
  addLabelOnlyMarkers(data = districts,
                    lng = ~long, lat = ~lat, label = ~lapply(group, html),
                    labelOptions = labelOptions(noHide = TRUE, direction = 'center', textOnly = TRUE)) %>%
  addLegend(position = "bottomright", pal = pal_districts, values = Listings_District$price_avg,
          title = "Average $/night")
```
