...
```{r}
berlin_district_sf <- berlin %>%
  st_as_sf() %>%
  sf::st_set_crs(proj4string(berlin)) %>%
  dplyr::rename(id = BEZNAME) %>%
  dplyr::group_by(id) %>%
  dplyr::summarize(do_union = TRUE) %>%
  dplyr::mutate(group = id,
                view  = "Districts")
...
#   sf::st_set_crs(sp::proj4string(berlin)) %>% # add Coordinate Reference System (CRM)
...
# Object where Buckow appears only once
...
vbb_AB_sf <- stations_df %>%
  dplyr::mutate(name = gsub("S ", "", name),
                name = gsub("U ", "", name)) %>%
  # Select from the list of all stations those that are railway and whose name belongs to our Ringbahn names DF
  dplyr::filter(fclass %like% "railway" & name %in% ringbahn_names_df$name) %>%
  dplyr::group_by(name) %>%
  dplyr::summarize(long = mean(long), 
                   lat  = mean(lat)) %>%
  # Rearrange according to the order we inserted before
...
leaflet() %>%
  ...
  addLabelOnlyMarkers(data = berlin_names %>% filter(group == "Neukölln", view == "Neighbourhoods"),
                    ...
                    labelOptions = labelOptions(noHide = TRUE, direction = 'center',
                                                textOnly = TRUE, textsize = "20px")) %>%
  addAwesomeMarkers(data = stations %>% filter(district == "Neukölln"),
             lng = ~long, lat = ~lat,
...
```{r}
rm("ringbahn_names_df", "berlin", "stations", "buckow", "buckow_sf", "berlin_neighbourhood_singlebuckow_sf", 
   "vbb_B", "berlin_all_sf")
```
