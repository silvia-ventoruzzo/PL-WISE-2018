correlation_plot = function(corr_df, variables = corr_df$variable, 
                            variables_original = TRUE) {
    ...
    if (all(correlation$variable == corr_df$variable)) {
      ...
      correlation = correlation %>%
        dplyr::filter(grepl(paste(variables, collapse = "|"), 
                            corr_df$variable))
    ...
    corr_plot = correlation %>%
      ...
      scale_fill_manual("sign", 
                        values = c("positive" = "green4", 
                                   "negative" = "red1", 
                                   "null" = "blue3")) +
      coord_flip() +
      ...
      theme(axis.text.y     = element_text(
                                color = ifelse(
                                          correlation$sign == "positive", 
                                               "green4",
                                        ifelse(
                                          correlation$sign == "negative", 
                                               "red1",
                                              "blue3"))),
