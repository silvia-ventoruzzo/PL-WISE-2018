...
bahn_stations_df <- stations %>%
  ...
  dplyr::mutate(name = gsub("Berlin ", "", name),
                ...
                name = gsub(" *\\(.*?\\) *", "", name),
                s_bahn = ifelse(startsWith(name, "S "), TRUE, FALSE),
...
vbb_AB_sf <- bahn_stations_df %>%
  # Select from the list of all train/subway stations those whose name belongs to our Ringbahn names DF
  dplyr::filter(name %in% ringbahn_names_df$name) %>%
