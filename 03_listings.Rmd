...
# listings_detailed_10 <- load_listings(file = "18.10_listings_detailed.csv")
# listings_summarized_10 <- load_listings(file = "18.10_listings_summary.csv")
# listings_detailed_11 <- load_listings(file = "07.11_listings_detailed.csv")
# listings_summarized_11 <- load_listings(file = "07.11_listings_summary.csv")
...
Compare df for October and November
```{r}
# p_load(arsenal)
# empty_rows <- listings_summarized_10 %>%
#   filter(row_number() %in% 1:(nrow(listings_summarized_11) - nrow(listings_summarized_10))) %>%
#   mutate_all(funs(replace(., values = NA)))
#   
# listings_summarized_10_empty_rows <- rbind(listings_summarized_10, empty_rows)
#   
# listings_summarized_10 
# comparison <- testthat::compare(listings_summarized_10_empty_rows$id, listings_summarized_11$id)
# 
# summary(comparison)
```
...
listings_check <- dplyr::full_join(listings_summarized, listings_detailed, by = "id") %>%
  dplyr::transmute(diff_neighborhood = 
                     ifelse(neighbourhood_group_cleansed == neighbourhood_group, TRUE, FALSE),
...
```{r}
listings <- listings %>%
  dplyr::mutate(review_scores_value = ifelse(number_of_reviews == 0, 0, review_scores_value),
...
listings <- listings %>%
  ...
  dplyr::mutate(beds = ifelse(is.na(beds) & !is.na(bedrooms), bedrooms, beds),
        ...
        # Since in this case the property can accomodate only one person, we assume that it has 1 bed and 1            bedroom (even if it might not be separate)
              ...
              bedrooms = ifelse(is.na(beds) & is.na(bedrooms), 1, bedrooms))
```
...
listings <- listings %>%
    ...
    dplyr::select(id, neighbourhood_group, lat, long, price, property_type, room_type,
                  ...
                  review_scores_value, number_of_reviews, reviewed_yn, cancellation_policy,
                  availability_30, availability_60, availability_90, availability_365)
```

## New features

Join with info from the calendar
...
rm("listings_month_availability", "listings_weekday_availability", 
   "listings_season_availability", "listings_daytype_availability")
