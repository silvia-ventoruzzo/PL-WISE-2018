...
server <- function(input, output) {
  ...
  output$main_map <- renderLeaflet({
    ...
    if (gsub(" ", "_", input$variable1) == "none") {
      ...
      # I want to change Tab with click on Leaflet Marker - NOT WORKING YET
      ...
      observeEvent(input$main_map_marker_click,{
        ...
        # if (input$main_map_marker_click) {
        updateTabItems(session, inputId = "tabs", selected = "district")
      ...
      output$secondary_map <- renderLeaflet({
        ...
        icons <- awesomeIconList(
          "properties" = makeAwesomeIcon(icon = "home", markerColor = "red"),
          "stations"   = makeAwesomeIcon(icon = "subway", library = "fa", markerColor = "green", iconColor = "#FFFFFF"),
          "sights"     = makeAwesomeIcon(icon = "eye-open", markerColor = "blue")
        )
      ...
      # checkboxInputs
      observeEvent({c(input$properties, input$stations, input$sights, input$district)}, {
        
        proxy <- leafletProxy("secondary_map")
        
        if (input$properties) {proxy %>% showGroup("properties")
        } else {proxy %>% hideGroup("properties")}
        
        if (input$stations) {proxy %>% showGroup("stations")
        } else {proxy %>% hideGroup("stations")}
        
        if (input$sights) {proxy %>% showGroup("sights")
        } else {proxy %>% hideGroup("sights")}
